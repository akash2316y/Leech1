...
class MirrorLeechListener:
    ...
    async def onDownloadComplete(self):
        ...
        # --- AUTO RENAME LOGIC START ---
        try:
            from bot.modules.auto_rename import apply_auto_rename
            
            # User ki personal rename format fetch ho raha hai (inline user settings se)
            user_rename_format = self.user_dict.get('rename')
            
            rename_base = up_path or dl_path
            if await aiopath.isfile(rename_base):
                original_filename = ospath.basename(rename_base)
                dir_path = ospath.dirname(rename_base)
                
                final_name = apply_auto_rename(original_filename, user_rename_format=user_rename_format)
                
                if original_filename != final_name:
                    new_file_path = ospath.join(dir_path, final_name)
                    await move(rename_base, new_file_path)
                    if up_path:
                        up_path = new_file_path
                    else:
                        dl_path = new_file_path
            elif await aiopath.isdir(rename_base):
                for dirpath, _, files in await sync_to_async(walk, rename_base):
                    for file_ in files:
                        final_name = apply_auto_rename(file_, user_rename_format=user_rename_format)
                        if file_ != final_name:
                            await move(ospath.join(dirpath, file_), ospath.join(dirpath, final_name))
        except Exception as e:
            LOGGER.error(f"Auto Rename Error: {e}")
        # --- AUTO RENAME LOGIC END ---
