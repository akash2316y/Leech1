...
desp_dict = {'rcc': ['RClone is a command-line program to sync files and directories to and from different cloud storage providers like GDrive, OneDrive...', 'Send rclone.conf. \n<b>Timeout:</b> 60 sec'],
            ...
            'rename': ['Auto Rename Format is used to automatically rename files using placeholders like [S{season}E{episode}] {title} [{quality}] {audio}.', 'Send Auto Rename Format.\n<b>Example:</b> <code>[S{season}E{episode}] {title} [{quality}] [{audio}]</code>\n<b>Timeout:</b> 60 sec'],
            }
...
async def get_user_settings(from_user, key=None, edit_type=None, edit_mode=None):
    ...
    if key is None:
        ...
        # Auto Rename Button
        rename_fmt = 'Not Exists' if (val:=user_dict.get('rename', '')) == '' else val
        buttons.ibutton(f"{'✅️' if rename_fmt != 'Not Exists' else ''} Auto Rename", f"userset {user_id} rename")
        ...
        text += f"\n\n➲ <b>Auto Rename Format :</b> <code>{escape(rename_fmt)}</code>"
        ...
        if key == 'rcc':
            ...
            set_exist = 'Not Exists' if (val:=user_dict.get('rename', '')) == '' else val
            text += f"➲ <b>Auto Rename Format :</b> <code>{escape(set_exist)}</code>\n\n"
        elif key == 'split_size':
...
async def user_settings(client, message):
    if len(message.command) > 1 and (message.command[1] == '-s' or message.command[1] == '-set'):
        ...
        if set_arg and (reply_to := message.reply_to_message):
            ...
            if set_arg in ['lprefix', 'lsuffix', 'lremname', 'lcaption', 'ldump', 'yt_opt', 'metadata', 'lattachment', 'rename'] and reply_to.text:
...
async def edit_user_settings(client, query):
    ...
    if user_id != int(data[1]):
        ...
        handler_dict[user_id] = False
        await query.answer()
        update_user_ldata(user_id, 'rename', '')
        await update_user_settings(query, 'rename', 'universal')
        if DATABASE_URL:
            await DbManger().update_user_data(user_id)
